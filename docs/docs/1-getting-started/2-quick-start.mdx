...
2. Open [AdvantageScope](https://docs.wpilib.org/en/stable/docs/software/dashboards/advantagescope.html)
3. Look for the `QuestNav` table in NetworkTables
4. Verify that `QuestNav/framedata/isTracking` = `true`
...
- [ ] Open your dashboard and verify `QuestNav/framedata/isTracking` = `true`
- [ ] Check `QuestNav/deviceData/battery_percent` is above 20%
...
- **`QuestNav.isConnected()` returns `false`**: Ensure the headset cameras have a clear view. Restart QuestNav if tracking doesn't recover.
